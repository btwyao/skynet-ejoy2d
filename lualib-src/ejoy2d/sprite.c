...
static uint16_t dynamic_tex_coord[8] = {0,1,1,1,1,0,0,0};
//static uint16_t dynamic_tex_coord[8] = {0,0,1,0,1,1,0,1};
...
void
sprite_drawquad(struct pack_picture *picture, int *dynamic_tex, const struct srt *srt,  const struct sprite_trans *arg) {
	struct matrix tmp;
	float vb[16];
	int i,j;
	if (arg->mat == NULL) {
	...
	for (i=0;i<picture->n;i++) {
		...
		int texid = q->texid;
		if (dynamic_tex && dynamic_tex[i] != -1)
			texid = dynamic_tex[i];
		...
		for (j=0;j<4;j++) {
			...
			float tx,ty;
			if (!dynamic_tex || dynamic_tex[i] == -1) {
				tx = q->texture_coord[j*2+0];
				ty = q->texture_coord[j*2+1];
				texture_coord(q->texid, &tx, &ty);
			} else {
				tx = dynamic_tex_coord[j*2+0];
				ty = dynamic_tex_coord[j*2+1];
			}
...
sprite_size(struct sprite_pack *pack, int id) {
	...
	if (type == TYPE_ANIMATION) {
		...
		struct pack_picture * pic = (struct pack_picture *)pack->data[id];
		return sizeof(struct sprite) + (pic->n - 1) * sizeof(int);
	} else {
...
int
sprite_child_index(struct sprite *parent, struct sprite *child) {
	if (parent->type != TYPE_ANIMATION)
		return -1;
	struct pack_animation *ani = parent->s.ani;
	int i;
	for (i=0;i<ani->component_number;i++) {
		if (parent->data.children[i] == child) {
			return i;
		}
	}
	return -1;
...
draw_child(struct sprite *s, struct srt *srt, struct sprite_trans * ts) {
	...
	case TYPE_PICTURE:
		...
		sprite_drawquad(s->s.pic, s->data.dynamic_tex, srt, t);
		return 0;
	case TYPE_POLYGON:
		switch_program(t, PROGRAM_PICTURE);
...
struct sprite_trans *
sprite_complete_trans(struct sprite *s, struct sprite_trans *temp, struct matrix *temp_matrix) {
	*temp = s->t;
	if (s->t.mat == NULL) {
	...
	temp->mat = temp_matrix;
	struct sprite_trans *trans = temp;
	struct sprite *temp_s = s;
	while(temp_s->parent) {
		struct sprite *parent = temp_s->parent;
		struct pack_animation *ani = parent->s.ani;
		struct sprite_trans *ct = NULL;
		int index;
		for (index=0;index<ani->component_number;index++) {
		...
		struct sprite_trans temp2 = *trans;
		struct matrix temp_matrix2= *trans->mat;
		temp2.mat = &temp_matrix2;
		trans = trans_mul(&temp2, ct, temp, temp_matrix);
		temp2 = *trans;
		temp_matrix2= *trans->mat;
		temp2.mat = &temp_matrix2;
		trans = trans_mul(&temp2, &parent->t, temp, temp_matrix);
...
const char *
sprite_dynamic_tex(struct sprite * s) {
	int i,j;
	float vb[16];
	struct pack_picture *pic = s->s.pic;
	int * texid = s->data.dynamic_tex;
	switch_program(&s->t, PROGRAM_PICTURE);
	for (i=0;i<pic->n;i++) {
		if (texid[i] == -1)
			continue;
		struct pack_quad *q = &pic->rect[i];
		int w = q->screen_coord[2] - q->screen_coord[0];
		int h = q->screen_coord[7] - q->screen_coord[1];
		const char *err = texture_new_rt(texid[i],w / SCREEN_SCALE,h / SCREEN_SCALE);
		if (err)
			return err;
		...
		if (glid == 0)
			continue;
		shader_texture(glid);
		int32_t screen_coord[8] = {0,0,2,0,2,-2,0,-2};
//		int32_t screen_coord[8] = {0,-2,2,-2,2,0,0,0};
		for (j=0;j<4;j++) {
			float vx = screen_coord[j*2+0];
			float vy = screen_coord[j*2+1];
		...
		texture_active_rt(texid[i]);
		shader_draw(vb, s->t.color);
		texture_inactive_rt();
	}
	return NULL;
}
